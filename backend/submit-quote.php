...
/**
 * Submit Quote API
 * Stores quote requests and sends admin/customer emails.
 */
...
try {
    ...
    $firstName = safeString($data, 'firstName', 'first_name');
    $lastName = safeString($data, 'lastName', 'last_name');
    $email = safeString($data, 'email');
    $phone = safeString($data, 'phone');
    $service = safeString($data, 'service');
    $projectDetails = safeString($data, 'message', 'project_details');

    if ($projectDetails === '') {
        $projectDetails = 'Quick quote request from homepage';
    }
    ...
    $company = nullableString($data, 'company', 'company_name');
    ...
    safeLog('Exception in submit-quote.php: ' . $exception->getMessage() . ' at line ' . $exception->getLine());
...
function safeLog($message) {
    try {
        ...
        error_log('submit-quote safeLog failure: ' . $exception->getMessage());
    }
}
...
function sendLeadNotificationEmails($quoteId, $lead) {
    ...
    $fromEmail = defined('SMTP_FROM_EMAIL') && SMTP_FROM_EMAIL !== '' ? SMTP_FROM_EMAIL : 'info@everythingeasy.in';
    $fromName = defined('SMTP_FROM_NAME') && SMTP_FROM_NAME !== '' ? SMTP_FROM_NAME : 'EverythingEasy Leads';
...
function safeSendCustomerThankYouMail($to, $subject, $plainBody, $fromEmail, $fromName) {
    ...
    if ($smtpConfigured) {
        $smtpOk = smtpSendPlainTextMail($to, $subject, $plainBody, $fromEmail, $fromName);
        if ($smtpOk) {
            return true;
        }
...
function safeSendPlainTextMail($to, $subject, $plainBody, $fromEmail, $fromName) {
    try {
        ...
        safeLog('safeSendPlainTextMail exception: ' . $exception->getMessage());
        return false;
    }
}
...
function safeSendHtmlMail($to, $subject, $htmlBody, $fromEmail, $fromName) {
    try {
        ...
        safeLog('safeSendHtmlMail exception: ' . $exception->getMessage());
        return false;
    }
}
